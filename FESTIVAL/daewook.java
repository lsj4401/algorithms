...
//		sc.close();
	...
	public static double calculateMinimumFeeAverage() {
		...
		System.out.println("init avg " + minFeeAvg);
		
		for (int i = 0; i < rentFee.length; i++) {
			System.out.print(rentFee[i] + " ");
		}
		System.out.println();
		...
		for (int i = mainBandsCount; i < rentFee.length; i++) {
			if (minFeeAvg >= rentFee[i]) {
				if (rentFee[i] < rentFee[i - cnt]) {
					minFeeAvg = Math.min(minFeeAvg, (double) recursiveSum(i, cnt) / cnt);
				} else {
					sum += rentFee[i];
					minFeeAvg = Math.min(minFeeAvg, (double) sum / ++cnt);
				}
			}
